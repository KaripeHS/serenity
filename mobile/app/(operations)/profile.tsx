...
export default function OperationsProfileScreen() {
    ...
    return (
        <ScrollView className="flex-1 bg-background">
            {/* Profile Header */}
            <View className="bg-info px-6 pt-8 pb-6 items-center rounded-b-3xl">
                <View className="h-24 w-24 bg-white/20 rounded-full items-center justify-center mb-4">
                    <Text className="text-3xl font-bold text-white">
                        {user?.firstName?.charAt(0) || 'O'}{user?.lastName?.charAt(0) || 'P'}
                    </Text>
                </View>
                <Text className="text-2xl font-bold text-white">
                    {user ? `${user.firstName || ''} ${user.lastName || ''}`.trim() : 'Operations User'}
                </Text>
                <Text className="text-blue-200 font-medium mt-1">
