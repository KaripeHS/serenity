            ...
            <View className="px-6 pt-8 pb-6 items-center rounded-b-3xl" style={{ backgroundColor: Colors.caregiver.DEFAULT }}>
                <View className="h-24 w-24 bg-white/20 rounded-full items-center justify-center mb-4">
                    <Text className="text-3xl font-bold text-white">
                        {user?.firstName?.charAt(0) || 'H'}{user?.lastName?.charAt(0) || 'R'}
                    </Text>
                </View>
                <Text className="text-2xl font-bold text-white">
                    {user ? `${user.firstName || ''} ${user.lastName || ''}`.trim() : 'HR User'}
                </Text>
                <Text className="text-purple-200 font-medium mt-1">
                    {getRoleDisplayName(user?.role || 'hr_director')}
                </Text>
                <Text className="text-purple-100 text-sm mt-1">{user?.email || ''}</Text>
            </View>
