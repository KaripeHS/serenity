/**
 * Finance Profile Screen - Reuses executive profile with finance styling
 */
...
export default function FinanceProfileScreen() {
    ...
    return (
        <ScrollView className="flex-1 bg-background">
            {/* Profile Header */}
            <View className="bg-success px-6 pt-8 pb-6 items-center rounded-b-3xl">
                ...
                <Text className="text-2xl font-bold text-white">
                    {user ? `${user.firstName || ''} ${user.lastName || ''}`.trim() : 'Finance User'}
                </Text>
                <Text className="text-green-200 font-medium mt-1">
                    {getRoleDisplayName(user?.role || 'finance_director')}
                </Text>
                <Text className="text-green-100 text-sm mt-1">{user?.email || ''}</Text>
            </View>
